import {
  ...
  import { isDocumentSymbol, parseDocumentSymbol, SymbolData } from './utils';
  ...
  class Provider implements CodeLensProvider {
	...
	async provideCodeLenses(document: TextDocument): Promise<CodeLens[] | null> {
	  ...
	  if (symbols === undefined) {
		window.showErrorMessage('Erro ao encontrar simbolos no arquivo aberto.');
		return null;
	  }
	  ...
	  const asyncResult = rangesList.map(async (data) => {
		...
		if (toResolve) {
		  return new CodeLens(range, {
			title: `"${name}" não possui referencias`,
			command: ''
		  });
		}
